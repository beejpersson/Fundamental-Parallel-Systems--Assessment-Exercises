	...
	void run(){
		...
		if (myPlayerId == -1) {
			...
			while (enroled) {
				...
				def turnCounter = gameDetails.turnCounter
				...
				while ((chosenPairs[1] == null) && (enroled) && (notMatched)) {
					nextPairConfig.write("WAITING FOR TURN")
					if (myPlayerId == turnCounter) { //check if it's player's turn
					...
					switch ( outerAlt.select() ) {
						case WITHDRAW:	
							...
							break	
						case CONTROLLER:
							def u = fromController.read()
							if(u instanceof Update){ //if board updates read in, display updated board
								def newUpdatedPoints = (Update)u
								def pairData1 = pairsMap.get(newUpdatedPoints.p1)
								changePairs(newUpdatedPoints.p1[0], newUpdatedPoints.p1[1], pairData1[1], pairData1[0])
								if(newUpdatedPoints.p2 != null){
									def pairData2 = pairsMap.get(newUpdatedPoints.p2)
									changePairs(newUpdatedPoints.p2[0], newUpdatedPoints.p2[1], pairData2[1], pairData2[0])
								}
						...
						case VALIDPOINT:
							...
							if ( matchOutcome == 2)  { // missmatching pair
								//send selected pair to controller
								toController.write(new Update( 	id: myPlayerId,
																gameId: gameId,
																p1: chosenPairs[0],
																p2: chosenPairs[1]))
								...
								switch (innerAlt.select()){
									case NEXT:
										...
										currentPair = 0
										toController.write(new NextTurn(id: myPlayerId)) //send playerid through to next turn manager, to increment turn
										notMatched = false
										break
									case WITHDRAW:
										withdrawButton.read()
										toController.write(new WithdrawFromGame(id: myPlayerId))
										toController.write(new NextTurn(id: myPlayerId))
										enroled = false
										break
								} // end inner switch
							} else if ( matchOutcome == 1) { //matching pair found
								notMatched = false
								toController.write(new ClaimPair ( id: myPlayerId,
												   	   			   gameId: gameId,
																   p1: chosenPairs[0],
																   p2: chosenPairs[1]))
							} else if ( matchOutcome == 0) {//only one selected
								// send first square picked through to the controller
								toController.write(new Update( 	id: myPlayerId,
																gameId: gameId,
																p1: chosenPairs[0],
																p2: chosenPairs[1]))
