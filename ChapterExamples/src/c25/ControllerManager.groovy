...
//			nPairs = generatePairsNumber(minPairs, pairsRange)
			...
			while (running) {
				...
				// increments turnCounter through players
				} else if (o instanceof NextTurn) {
					//sends signal for game to reload
					for (i in  0..< toPlayers.size())
						if(toPlayers[i] != null && i != turnCounter){
							toPlayers[i].write(o)
						}
					//increment turn counter through player's IDs
					def nextTurn = turnCounter
					for (i in  turnCounter..< toPlayers.size())
						if(toPlayers[i] != null && i > turnCounter){
							nextTurn = i
							break
						}
					//when on final player, return to first player
					if(nextTurn == turnCounter)
						nextTurn = 0
					turnCounter = nextTurn
				} else if ( o instanceof ClaimPair) {
					def claimPair = (ClaimPair)o
					for (i in  0..< toPlayers.size()) //write claim pair to players
						if(toPlayers[i] != null && i != turnCounter){
							toPlayers[i].write(2)
						}
				...
				} else if ( o instanceof Update) { // write update values to players
					for (i in  0..< toPlayers.size())
						if(toPlayers[i] != null && i != turnCounter){
							toPlayers[i].write(o)
						}
